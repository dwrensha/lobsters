...
const pkgdef :Spk.PackageDefinition = (
  ...
  alwaysInclude = ["usr/bin/node", "app", "config", "public"],
...
const startCommand :Spk.Manifest.Command = (
  ...
  argv = ["/sandstorm-http-bridge", "10000", "--", "/bin/bash", "start.bash"],
  environ = [
...
const continueCommand :Spk.Manifest.Command = (
  argv = ["/sandstorm-http-bridge", "10000", "--", "/bin/bash", "continue.bash"],
  environ = [
    (key = "PATH", value = "/usr/local/bin:/usr/bin:/bin")
  ]
);
